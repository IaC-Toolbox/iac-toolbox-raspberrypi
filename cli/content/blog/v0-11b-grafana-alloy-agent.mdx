...
**Swap panels remain N/A on macOS.** macOS only exposes swap as page counters (`node_memory_swapins_total`, `node_memory_swapouts_total`), not as byte-sized gauges. There is no equivalent for `node_memory_SwapTotal_bytes`. This is a macOS kernel limitation — swap size panels will always be N/A for Mac devices.
