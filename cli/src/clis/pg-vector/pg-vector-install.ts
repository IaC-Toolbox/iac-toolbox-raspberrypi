...
export async function runPgVectorInstall(
  destination: string,
  profile: string,
  filePath?: string
): Promise<void> {
  const credentials = loadCredentials(profile);
  const pgvectorPassword = credentials['pgvector_password'];
  ...
  try {
    ...
    // ── Ansible Invocation ────────────────────────────────────
    print.step('Deploying pgvector database...');
    print.divider();

    status = runAnsiblePlaybook('pgvector.yml', {
      ansibleDir: resolveAnsibleDir(destination),
      filePath: tmpFile,
      projectRoot: resolveProjectRoot(),
      env: { ...process.env },
      extraVars: { pgvector_password: pgvectorPassword },
    });
  } finally {
    unlinkSync(tmpFile);
  }
  ...
  if (status !== 0) {
    print.blank();
    print.step('pgvector install failed');
    print.pipe();
    print.error('Ansible playbook exited with errors');
    print.pipe('Check output above for details');
    print.pipe();
    print.pipe('To retry: iac-toolbox pg-vector install');
    print.closeError();
    process.exit(status ?? 1);
  }

  const port = config.pg_vector?.port ?? 5433;
  const database = config.pg_vector?.database ?? 'pgvector';

  print.blank();
  print.step('pgvector is running');
  print.pipe();
  print.pipe('Host            localhost');
  print.pipe(`Port            ${port}`);
  print.pipe(`Database        ${database}`);
  print.pipe('User            postgres');
  print.pipe('Extension       vector (pgvector)');
  print.pipe();
