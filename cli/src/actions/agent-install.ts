...
export async function runAgentInstall(
  ...
  const config = loadAgentConfig(destination);
  ...
  if (!prometheusRemoteWriteUrl) {
    ...
    print.pipe();
    print.pipe(
      'Run `iac-toolbox agent init` first to set the remote endpoints.'
    );
    print.closeError();
    process.exit(1);
  }
  ...
  if (result.status !== 0) {
    ...
    print.pipe();
    print.pipe('To retry: iac-toolbox agent apply');
    print.closeError();
