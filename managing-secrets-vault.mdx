...
Applications running on your Raspberry Pi need sensitive data like API keys, database passwords, and authentication tokens. In this guide, we'll set up HashiCorp Vault - a production-grade secrets management system that's more secure than storing secrets in plain text files.
...
All the Vault configuration and deployment scripts from this guide are available in the example repository. Feel free to use them as a starting point!

## Why HashiCorp Vault?

Let's compare it to the Ansible Vault approach:

**Security Improvements:**
- **No plain text on disk** - secrets never written to `/etc/raspberrypi.env`
- **Secrets fetched on-demand** - applications request secrets at runtime
- **Encryption at rest** - Vault data is encrypted when sealed
- **Dynamic secrets** - can generate credentials with TTLs
- **Audit logging** - tracks every secret access

**Developer Experience:**
- **Web UI** - manage secrets visually
- **Versioning** - track secret changes over time
- **API-first** - easy integration with any language
- **No manual seeding** - update secrets directly in Vault UI
...
│  │  - Auto-unseal on boot           │   │     vault.iac-toolbox.com
│  └──────────────────────────────────┘   │
│           ▲                              │
...
ok: [raspberrypi] => {
    "msg": [
        ...
        "Root Token: hvs.PX0QgVNFE3JvUDlMmRyxJh72",
        "",
        "IMPORTANT: Save these credentials securely!",
        "Unseal Key: uc+oV1vGa1sMw/pa4B/RW+K9Dd8sK3SYoftyIny0nx0=",
        "",
